...
### Requirement: Secretless Git behavior requires no signing credential
The `container` and `debian-dev-headless` Git configuration SHALL disable
commit signing and SHALL NOT declare a signing key or credential helper that
requires personal authentication material.

#### Scenario: Debian headless Git configuration is rendered
- **WHEN** the role is `debian-dev-headless`
- **THEN** `commit.gpgsign` is false and no signing key is emitted
